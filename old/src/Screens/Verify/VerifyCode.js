...
import { ResendCode_API, Verify_API } from "../../APIs/API"
...
const VerifyCode = () => {
  ...
  const VerifyMutation = useMutation("Verify_API", {
    mutationFn: Verify_API,
    onMutate: () => {
      setIsLoading(true)
    },
    onError: () => {
    ...
    onSuccess: (data) => {
      setIsLoading(false)
      if (data.data.status) {
        navigate(`/complete-profile/${data?.data?.token}/${user?.id}`, {
  ...
  const ResendMutation = useMutation("ResendCode_API", {
